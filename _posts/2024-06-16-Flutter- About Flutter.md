...
Dart 언어는 Google에서 개발한, 주로 모바일 애플리케이션 및 웹개발에 사용되는 언어로써 Dart는 특히 Flutter와 함께 사용될 때 강력한 기능을 발휘하며, 그 성능과 새앗ㅇ서면에서 주목받고 있다. 하지만 개발 커뮤니티와 라이브러리 생태계가 계속 성장해 나갈 것인지는 앞으로의 발전에 달려있다.
...
4. 멀티패러다임
Dart는 함수형 프로그래밍과 개체 지향 프로그래밍의 요소를 모두 포함하고 있어 다양한 프로그래밍 스타일을 지원한다.
...
3. __Hot Reload__
앱 개발 시 수정사항을 즉시 확인할 수 있는 Hot Reload 기능은 생상선을 향상시키는 데 큰 기여를 한다.
